(function($) {
    ...
    fetch('/data-in-js/month-stats')
    .then(response => response.json())
    .then(fetch_json => {
        if (activity !== null) {
        var activityData = [{
                first: fetch_json.data_monthly_page.print
            },
            {
                first: fetch_json.data_monthly_page.scan
            },
            {
                first: fetch_json.data_monthly_page.copies
            },
            {
                first: [25, 88, 25, 50, 70, 70, 60, 70, 50, 60, 50, 70, 35, 18, 15, 35, 40, 20, 30, 25, 22, 20, 45, 35]
            }
        ];
        activity.height = 300;
        ...
        var config = {
            type: "bar",
            data: {
                labels: fetch_json.data_monthly_page.day,
                datasets: [
					{
						label: "Страниц",
						data:  fetch_json.data_monthly_page.print,
...
if (jQuery('#chart_widget_2_1').length > 0) {
    fetch('/data-in-js/week-stats')
        .then(response => response.json())
        .then(fetch_json => {
            const chartsData = [
                { id: "chart_widget_2_1", data: fetch_json.data_weekly_stats.total },
                { id: "chart_widget_2_2", data: fetch_json.data_weekly_stats.print },
                { id: "chart_widget_2_3", data: fetch_json.data_weekly_stats.scan },
                { id: "chart_widget_2_4", data: fetch_json.data_weekly_stats.copies }
            ];

            chartsData.forEach(chart => {
                if (jQuery(`#${chart.id}`).length > 0) {
                    const ctx = document.getElementById(chart.id).getContext('2d');
                    const gradientStroke = ctx.createLinearGradient(0, 0, 0, 250);
                    gradientStroke.addColorStop(0, "#a0bfff");
                    gradientStroke.addColorStop(1, "#a0bfff");
